...
<head>
    ...
    <title>@yield('title', 'Artist Dashboard') - MusicStream</title>
...
<body class="bg-gray-900">
    <div class="flex h-screen">
        ...
        <div class="w-64 bg-gray-800 text-white">
            <div class="p-4">
                <h1 class="text-xl font-bold">MusicStream Artist</h1>
            </div>
            
            <nav class="mt-8">
                <a href="{{ route('artist.dashboard') }}" class="block px-4 py-2 {{ request()->is('artist/dashboard') ? 'bg-gray-900' : 'hover:bg-gray-700' }}">
                    Dashboard
                </a>
                <a href="{{ route('albums.index') }}" class="block px-4 py-2 {{ request()->is('albums*') ? 'bg-gray-900' : 'hover:bg-gray-700' }}">
                    Albums
                </a>
                <a href="{{ route('songs.index') }}" class="block px-4 py-2 {{ request()->is('songs*') ? 'bg-gray-900' : 'hover:bg-gray-700' }}">
                    Songs
                </a>
            </nav>
